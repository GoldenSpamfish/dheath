...
//-----------------------------------------------------------------
   ...
   public String toString()
   {
      ...
      //report += "Average cost: " +  movies.getAvgYear());
      report += "\n\nDVD List:\n\n";
   ...
   acc+= collection[i].getYear();
    }
   return (acc/count);    
   }
   
   public String directorReport(String DirectorName){
   
    for (int i=0; i<count; i++){
       if (DVDCollection [i].getDirector == DirectorName)
       movs += collection[i].getDirector();
    }
   return "movs";    
   }
}